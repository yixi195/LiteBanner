...
public class CoverFlowLayoutManger extends RecyclerView.LayoutManager {
     ...
     * 超过设置数量，则动态计算来获取
     */
    private final int MAX_RECT_COUNT = 10;
     ...
     * Item间隔与item宽的比例
     */
    private float mIntervalRatio = 0.12f;
     ...
     * 滑动速度  0 ~ 1
     */
    private double speedRatio = 0.98;
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        mStartX = Math.round(0); //(getHorizontalSpace() - mDecoratedChildWidth) * 1.0f / 2
        //计算第一个Item Y轴的启始位置坐标，这里为控件竖直方向居中
        mStartY = Math.round((getVerticalSpace() - mDecoratedChildHeight) * 1.0f  / 2);

        ////Logger.i(TAG,"mStartX===" + mStartX + "|mStartY==" + mStartY);
    ...
    private void layoutItems(RecyclerView.Recycler recycler,
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (!Rect.intersects(displayFrame, rect)) {//Item没有在显示区域，就说明需要回收
                ...
                layoutItem(child, rect, i); //更新Item位置
...
//                    Log.i("ysl","向右滚动===");
     ...
     * @param frame 位置信息
     */
    private void layoutItem(View child, Rect frame, int position) {
        ...
        if (!mIsFlatFlow) { //不是平面普通滚动的情况下才进行缩放
            //Log.i("ysl", "frame.left==" + frame.left + " | scaleX:" + computeScale(frame.left - mOffsetAll));
//            child.setScaleX(computeScale(frame.left - mOffsetAll)); //缩放
//            child.setScaleY(computeScale(frame.left - mOffsetAll)); //缩放

            float scaleX = computeScale(frame.left - mOffsetAll);
            float scaleY = computeScaleY(frame.left - mOffsetAll);

            //child.setPivotX(300);
            float pivotY = mDecoratedChildHeight - 150;
            ////Logger.i(TAG,"pivotY==" + pivotY);

            child.setPivotY(pivotY);
            child.setScaleX(scaleX); //缩放
            child.setScaleY(scaleY); //缩放
        }
...
//        }
        ...
        //Logger.i(TAG,"x==" + x  + " |mStartX:" + mStartX + " |mDecoratedChildWidth==" + mDecoratedChildWidth + " |scan==" + scale);
        return scale;
    }

    private float computeScaleY(int x){
        float scale = 1 - Math.abs(x - mStartX) * 1.0f / Math.abs(mStartX + mDecoratedChildWidth / mIntervalRatio);
        if (scale < 0) scale = 0;
        if (scale > 1) scale = 1;

        if (scale <= 0.99f){
            scale = 1 - Math.abs(x - mStartX) * 1.0f / Math.abs(mStartX + mDecoratedChildWidth / 0.35f);
            scale = scale - 0.4688f;
        }

        //Logger.i(TAG,"y==" + x + " |scan==" + scale);
        return scale;
    }
    ...
    /**
     * 获取中间位置
     * <p>Note:该方法主要用于{@link LBannerView#getChildDrawingOrder(int, int)}判断中间位置
     * <p>如果需要获取被选中的Item位置，调用{@link #getSelectedPos()}
     */
    public int getCenterPosition() {
        Log.i("ysl", "getCenterPosition===");
